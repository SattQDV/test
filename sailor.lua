...
local Tables = {
  ...
  IslandList = {"Starter", "Jungle", "Desert", "Snow", "Sailor", "Shibuya", "Hollow", "Boss", "Dungeon", "Shinjuku", "Valentine", "Slime", "Academy", "Judgement", "SoulSociety", "Ninja", "Lawless", "Tower"},
...
local function SendSafetyWebhook(targetPlayer, reason)
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "celina • " .. os.date("%x %X") }
        }}
    }
...
local function Func_AutoSkill()
    ...
    while task.wait() do
        ...
        if target and target.Parent then
            local npcName = target.Name
            local isRealBoss = npcName:find("Boss") and not table.find(Tables.MiniBossList, npcName)
            if isRealBoss then canExecute = false end
        end
        if canExecute and Toggles.AutoSkill_BossOnly.Value then
...
task.spawn(function()
    while task.wait(5) do
        if not getgenv().celina_Running then break end
...
ThemeManager:SetFolder("celina")
SaveManager:SetFolder("celina/SailorPiece")
